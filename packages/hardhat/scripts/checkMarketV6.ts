...
import {PrecogMasterV6, PrecogToken} from "../typechain-types";
...
async function main() {
    ...
    const contractAddress: string = '0x16D24dE99e3282F153B72229a3c23959cC20FdA3';  // PrecogMasterV6
    ...
    const master: PrecogMasterV6 = await ethers.getContractAt('PrecogMasterV6', contractAddress);
    ...
    const marketStart = createdMarket[3];
    const marketEnd = createdMarket[4];
    const marketCreatorAddress = createdMarket[5];
    const marketAddress = createdMarket[6];
    ...
    console.log(`\t       At: ${marketAddress} [PrecogMarketV6]`);
...
async function getMaxBuyShares(master: PrecogMasterV6, marketId: number, marketOutcome: number, balance: bigint) {
